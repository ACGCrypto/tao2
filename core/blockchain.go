...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	...
	// for waihui
	resultTrade, _ := lru.New(waihui_state.OrderCacheLimit)
	rejectedOrders, _ := lru.New(waihui_state.OrderCacheLimit)
		...
		if ok {
			...
			if bc.Config().IsTIPWaihui(currentBlock.Number()) && waihuiService != nil {
				...
				if err != nil {
					...
					if waihuiService.GetStateCache() != nil {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*waihui_state.WaihuiStateDB, error) {
	engine, ok := bc.Engine().(*posv.Posv)
	if ok {
		waihuiService := engine.GetWaihuiService()
	...
	return nil, errors.New("Get waihui state fail")
...
func (bc *BlockChain) Stop() {
	...
	if !bc.cacheConfig.Disabled {
		var waihuiTriedb *trie.Database
		engine, _ := bc.Engine().(*posv.Posv)
		triedb := bc.stateCache.TrieDB()
		if bc.Config().IsTIPWaihui(bc.CurrentBlock().Number()) && engine != nil {
			if waihuiService := engine.GetWaihuiService(); waihuiService != nil && waihuiService.GetStateCache() != nil {
				waihuiTriedb = waihuiService.GetStateCache().TrieDB()
			}
		}
		...
		if bc.Config().IsTIPWaihui(bc.CurrentBlock().Number()) && engine != nil && waihuiTriedb != nil {
			if waihuiService := engine.GetWaihuiService(); waihuiService != nil && waihuiService.GetTriegc() != nil {
				for !waihuiService.GetTriegc().Empty() {
					waihuiTriedb.Dereference(waihuiService.GetTriegc().PopItem().(common.Hash), common.Hash{})
				}
			}
		}
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, waihuiState *waihui_state.WaihuiStateDB) (status WriteStatus, err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
	...
	waihuiRoot := common.Hash{}
	if waihuiState != nil {
		waihuiRoot, err = waihuiState.Commit()
		if err != nil {
			return NonStatTy, err
		}
	}
	...
	if bc.cacheConfig.Disabled {
		...
		if waihuiTrieDb != nil {
			if err := waihuiTrieDb.Commit(waihuiRoot, false); err != nil {
				return NonStatTy, err
			}
		}
	} else {
		...
		if bc.Config().IsTIPWaihui(block.Number()) && engine != nil {
			if waihuiTrieDb != nil {
				waihuiTrieDb.Reference(waihuiRoot, common.Hash{})
			}
			if waihuiService := engine.GetWaihuiService(); waihuiService != nil {
				waihuiService.GetTriegc().Push(waihuiRoot, -float32(block.NumberU64()))
			}
		}
		if current := block.NumberU64(); current > triesInMemory {
			...
			if size > limit || bc.gcproc > bc.cacheConfig.TrieTimeLimit {
				...
				if chosen >= lastWrite+triesInMemory || size >= 2*limit || bc.gcproc >= 2*bc.cacheConfig.TrieTimeLimit {
					...
					lastWrite = chosen
					bc.gcproc = 0
					if bc.Config().IsTIPWaihui(block.Number()) && waihuiTrieDb != nil {
						waihuiTrieDb.Commit(oldWaihuiRoot, true)
					}
				}
			}
			...
			if bc.Config().IsTIPWaihui(block.Number()) && engine != nil {
				if waihuiService := engine.GetWaihuiService(); waihuiService != nil {
					for !waihuiService.GetTriegc().Empty() {
						tomoRoot, number := waihuiService.GetTriegc().Pop()
						if uint64(-number) > chosen {
							waihuiService.GetTriegc().Push(tomoRoot, number)
							break
						}
						waihuiTrieDb.Dereference(tomoRoot.(common.Hash), common.Hash{})
					}
				}
			}
		}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		if bc.Config().IsTIPWaihui(block.Number()) && engine != nil {
			if waihuiService := engine.GetWaihuiService(); waihuiService != nil {
				...
				for _, txMatchBatch := range txMatchBatchData {
					...
					err := bc.Validator().ValidateMatchingOrder(statedb, waihuiState, txMatchBatch, author)
					if err != nil {
						bc.reportBlock(block, nil, err)
						return i, events, coalescedLogs, err
					}
		...
		status, err := bc.WriteBlockWithState(block, receipts, statedb, waihuiState)
		if err != nil {
			return i, events, coalescedLogs, err
		}
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPWaihui(block.Number()) {
...
func (bc *BlockChain) getResultBlock(block *types.Block, verifiedM2 bool) (*ResultProcessBlock, error) {
	...
	return &ResultProcessBlock{receipts: receipts, logs: logs, state: statedb, waihuiState: waihuiState, proctime: proctime, usedGas: usedGas}, nil
}
...
func (bc *BlockChain) insertBlock(block *types.Block) ([]interface{}, []*types.Log, error) {
	...
	status, err := bc.WriteBlockWithState(block, result.receipts, result.state, result.waihuiState)
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if bc.chainConfig.IsTIPWaihui(commonBlock.Number()) {
...
func (bc *BlockChain) logExchangeData(block *types.Block) {
	...
	waihuiService := engine.GetWaihuiService()
	if waihuiService == nil || !waihuiService.IsSDKNode() {
		return
	}
	txMatchBatchData, err := ExtractMatchingTransactions(block.Transactions())
	if err != nil {
	...
	for _, txMatchBatch := range txMatchBatchData {
		...
		for _, txMatch := range txMatchBatch.Data {
			var (
				takerOrderInTx *waihui_state.OrderItem
				trades         []map[string]string
				rejectedOrders []*waihui_state.OrderItem
			)
			...
			if ok && rejected != nil {
				rejectedOrders = rejected.([]*waihui_state.OrderItem)
			}
			...
			if err := waihuiService.SyncDataToSDKNode(takerOrderInTx, txMatchBatch.TxHash, txMatchTime, currentState, trades, rejectedOrders, &dirtyOrderCount); err != nil {
...
func (bc *BlockChain) reorgTxMatches(deletedTxs types.Transactions, newChain types.Blocks) {
	...
	for _, deletedTx := range deletedTxs {
		if deletedTx.IsMatchingTransaction() {
			...
			waihuiService.RollbackReorgTxMatch(deletedTx.Hash())
		}
	}
...
func (bc *BlockChain) AddMatchingResult(txHash common.Hash, matchingResults map[common.Hash]waihui_state.MatchingResult) {
