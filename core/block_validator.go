...
func (v *BlockValidator) ValidateMatchingOrder(statedb *state.StateDB, waihuiStatedb *waihui_state.WaihuiStateDB, txMatchBatch waihui_state.TxMatchBatch, coinbase common.Address) error {
	posvEngine, ok := v.bc.Engine().(*posv.Posv)
	if posvEngine == nil || !ok {
		return ErrNotPoSV
	}
	waihuiService := posvEngine.GetWaihuiService()
	if waihuiService == nil {
		return fmt.Errorf("waihui not found")
	}
	log.Debug("verify matching transaction found a TxMatches Batch", "numTxMatches", len(txMatchBatch.Data))
	matchingResult := map[common.Hash]waihui_state.MatchingResult{}
	for _, txMatch := range txMatchBatch.Data {
		...
		newTrades, newRejectedOrders, err := waihuiService.ApplyOrder(coinbase, v.bc, statedb, waihuiStatedb, waihui_state.GetOrderBookHash(order.BaseToken, order.QuoteToken), order)
		if err != nil {
			return err
		}
		matchingResult[order.Hash] = waihui_state.MatchingResult{
			Trades:  newTrades,
			Rejects: newRejectedOrders,
		}
	}
	if waihuiService.IsSDKNode() {
...
func ExtractMatchingTransactions(transactions types.Transactions) ([]waihui_state.TxMatchBatch, error) {
	txMatchBatchData := []waihui_state.TxMatchBatch{}
	for _, tx := range transactions {
		if tx.IsMatchingTransaction() {
			txMatchBatch, err := waihui_state.DecodeTxMatchesBatch(tx.Data())
			if err != nil {
				return []waihui_state.TxMatchBatch{}, fmt.Errorf("transaction match is corrupted. Failed to decode txMatchBatch. Error: %s", err)
			}
