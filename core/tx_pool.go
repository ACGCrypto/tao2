...
type TxPool struct {
	...
	trc2FeeCapacity map[common.Address]*big.Int
}
...
func (pool *TxPool) reset(oldHead, newHead *types.Header) {
	...
	pool.trc2FeeCapacity = state.GetTRC2FeeCapacityFromStateWithCache(newHead.Root, statedb)
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if tx.To() != nil {
		if value, ok := pool.trc2FeeCapacity[*tx.To()]; ok {
			feeCapacity = value
			if !state.ValidateTRC2Tx(pool.pendingState.StateDB, from, *tx.To(), tx.Data()) {
				return ErrInsufficientFunds
			}
			cost = tx.TRC2Cost()
			minGasPrice = common.TRC2GasPrice
		}
	}
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	for _, addr := range accounts {
		...
		drops, _ := list.Filter(pool.currentState.GetBalance(addr), pool.currentMaxGas, pool.trc2FeeCapacity)
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(pool.currentState.GetBalance(addr), pool.currentMaxGas, pool.trc2FeeCapacity)
		for _, tx := range drops {
			hash := tx.Hash()
