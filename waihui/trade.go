...
func (t *Trade) SetBSON(raw bson.Raw) error {
	...
	t.Amount = waihui_state.ToBigInt(decoded.Amount)
	t.PricePoint = waihui_state.ToBigInt(decoded.PricePoint)

	t.MakeFee = waihui_state.ToBigInt(decoded.MakeFee)
	t.TakeFee = waihui_state.ToBigInt(decoded.TakeFee)
