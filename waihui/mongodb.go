...
func (db *MongoDatabase) GetObject(hash common.Hash, val interface{}) (interface{}, error) {
	...
	if cached, ok := db.cacheItems.Get(cacheKey); ok {
		...
		switch val.(type) {
		case *waihui_state.OrderItem:
			var oi *waihui_state.OrderItem
...
func (db *MongoDatabase) CommitOrder(o *waihui_state.OrderItem) error {
...
func (db *MongoDatabase) GetOrderByTxHash(txhash common.Hash) []*waihui_state.OrderItem {
	var result []*waihui_state.OrderItem
	sc := db.Session.Copy()
	defer sc.Close()
...
func (db *MongoDatabase) GetListOrderByHashes(hashes []string) []*waihui_state.OrderItem {
	var result []*waihui_state.OrderItem
	sc := db.Session.Copy()
	defer sc.Close()
