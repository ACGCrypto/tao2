...
func NewBatchDatabaseWithEncode(datadir string, cacheLimit int) *BatchDatabase {
	...
	batchDB := &BatchDatabase{
		...
		emptyKey:   waihui_state.EmptyKey(), // pre alloc for comparison
...
func (db *BatchDatabase) GetOrderByTxHash(txhash common.Hash) []*waihui_state.OrderItem {
	return []*waihui_state.OrderItem{}
}

func (db *BatchDatabase) GetListOrderByHashes(hashes []string) []*waihui_state.OrderItem {
	return []*waihui_state.OrderItem{}
}
