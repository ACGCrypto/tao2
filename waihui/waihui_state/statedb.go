...
func New(root common.Hash, db Database) (*WaihuiStateDB, error) {
	tr, err := db.OpenTrie(root)
	if err != nil {
		return nil, err
	}
	return &WaihuiStateDB{
		db:                       db,
		trie:                     tr,
...
func (self *WaihuiStateDB) setError(err error) {
	if self.dbErr == nil {
		self.dbErr = err
	}
}

func (self *WaihuiStateDB) Error() error {
	return self.dbErr
}

// Exist reports whether the given orderId address exists in the state.
// Notably this also returns true for suicided exchanges.
func (self *WaihuiStateDB) Exist(addr common.Hash) bool {
	return self.getStateExchangeObject(addr) != nil
}

// Empty returns whether the state object is either non-existent
// or empty according to the EIP161 specification (balance = nonce = code = 0)
func (self *WaihuiStateDB) Empty(addr common.Hash) bool {
	so := self.getStateExchangeObject(addr)
	return so == nil || so.empty()
}

func (self *WaihuiStateDB) GetNonce(addr common.Hash) uint64 {
...
func (self *WaihuiStateDB) GetPrice(addr common.Hash) *big.Int {
	stateObject := self.getStateExchangeObject(addr)
	if stateObject != nil {
...
func (self *WaihuiStateDB) Database() Database {
	return self.db
}

func (self *WaihuiStateDB) SetNonce(addr common.Hash, nonce uint64) {
...
func (self *WaihuiStateDB) SetPrice(addr common.Hash, price *big.Int) {
	stateObject := self.GetOrNewStateExchangeObject(addr)
	if stateObject != nil {
...
func (self *WaihuiStateDB) InsertOrderItem(orderBook common.Hash, orderId common.Hash, order OrderItem) {
...
func (self *WaihuiStateDB) GetOrder(orderBook common.Hash, orderId common.Hash) OrderItem {
...
func (self *WaihuiStateDB) SubAmountOrderItem(orderBook common.Hash, orderId common.Hash, price *big.Int, amount *big.Int, side string) error {
...
func (self *WaihuiStateDB) CancelOrder(orderBook common.Hash, order *OrderItem) error {
...
func (self *WaihuiStateDB) GetVolume(orderBook common.Hash, price *big.Int, orderType string) *big.Int {
	stateObject := self.GetOrNewStateExchangeObject(orderBook)
...
func (self *WaihuiStateDB) GetBestAskPrice(orderBook common.Hash) (*big.Int, *big.Int) {
...
func (self *WaihuiStateDB) GetBestBidPrice(orderBook common.Hash) (*big.Int, *big.Int) {
	stateObject := self.getStateExchangeObject(orderBook)
	if stateObject != nil {
...
func (self *WaihuiStateDB) GetBestOrderIdAndAmount(orderBook common.Hash, price *big.Int, side string) (common.Hash, *big.Int, error) {
	stateObject := self.GetOrNewStateExchangeObject(orderBook)
	if stateObject != nil {
		var stateOrderList *stateOrderList
		switch side {
...
func (self *WaihuiStateDB) updateStateExchangeObject(stateObject *stateExchanges) {
...
func (self *WaihuiStateDB) getStateExchangeObject(addr common.Hash) (stateObject *stateExchanges) {
	// Prefer 'live' objects.
...
func (self *WaihuiStateDB) setStateExchangeObject(object *stateExchanges) {
	self.stateExhangeObjects[object.Hash()] = object
	self.stateExhangeObjectsDirty[object.Hash()] = struct{}{}
}

// Retrieve a state object or create a new state object if nil.
func (self *WaihuiStateDB) GetOrNewStateExchangeObject(addr common.Hash) *stateExchanges {
...
func (self *WaihuiStateDB) MarkStateExchangeObjectDirty(addr common.Hash) {
...
func (self *WaihuiStateDB) createExchangeObject(hash common.Hash) (newobj *stateExchanges) {
...
func (self *WaihuiStateDB) Copy() *WaihuiStateDB {
	self.lock.Lock()
...
func (s *WaihuiStateDB) clearJournalAndRefund() {
	s.journal = nil
	s.validRevisions = s.validRevisions[:0]
}

// Snapshot returns an identifier for the current revision of the state.
func (self *WaihuiStateDB) Snapshot() int {
...
func (self *WaihuiStateDB) RevertToSnapshot(revid int) {
...
func (s *WaihuiStateDB) IntermediateRoot() common.Hash {
	s.Finalise()
	return s.trie.Hash()
}

// Commit writes the state to the underlying in-memory trie database.
func (s *WaihuiStateDB) Commit() (root common.Hash, err error) {
	...
	log.Debug("Waihui Trie cache stats after commit", "misses", trie.CacheMisses(), "unloads", trie.CacheUnloads(), "root", root.Hex())
	return root, err
}
