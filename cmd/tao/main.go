...
const (
	clientIdentifier = "tao" // Client identifier to advertise over the network
)
...
var (
	...
	nodeFlags = []cli.Flag{
		...
		utils.WaihuiEnabledFlag,
		utils.WaihuiDataDirFlag,
		utils.WaihuiDBEngineFlag,
		utils.WaihuiDBConnectionUrlFlag,
		utils.WaihuiDBReplicaSetNameFlag,
		utils.WaihuiDBNameFlag,
...
func init() {
	// Initialize the CLI app and start tao
	app.Action = tao
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright (c) 2020 Tao Network"
...
// tao is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func tao(ctx *cli.Context) error {
