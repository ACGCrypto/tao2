...
func main() {
	...
	trc2IssuerAddr, trc2Issuer, err := waihui.DeployTRC2Issuer(auth, client, simulation.MinTRC2Apply)
	if err != nil {
		log.Fatal("DeployTRC2Issuer", err)
	}
	trc2Issuer.TransactOpts.GasPrice = big.NewInt(210000000000000)

	fmt.Println("===> trc2 issuer address", trc2IssuerAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : TRC Issuer")
	time.Sleep(2 * time.Second)

	//init WAIHUI Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	waihuiListtingAddr, waihuiListing, err := waihui.DeployWAIHUIListing(auth, client)
	if err != nil {
		log.Fatal("DeployWAIHUIListing", err)
	}
	waihuiListing.TransactOpts.GasPrice = big.NewInt(210000000000000)

	fmt.Println("===> waihui listing address", waihuiListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : waihui listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := waihui.DeployRelayerRegistration(auth, client, waihuiListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	tokenList := initTRC2(auth, client, currentNonce, simulation.TokenNameList)

	currentNonce = currentNonce + uint64(len(simulation.TokenNameList)) // init smartcontract

	applyIssuer(trc2Issuer, tokenList, currentNonce)

	currentNonce = currentNonce + uint64(len(simulation.TokenNameList))
	applyWaihuiListing(waihuiListing, tokenList, currentNonce)
	...
	relayerRegistration, err = waihui.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	*/
	...
	fmt.Println("wait 10s to apply token to list waihui")
	time.Sleep(2 * time.Second)
}

func initTRC2(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		tokenAddr, _, err := waihui.DeployTRC2(auth, client, tokenName, tokenName, d, simulation.TRC2TokenCap, simulation.TRC2TokenFee)
		if err != nil {
			log.Fatal("DeployTRC2 ", tokenName, err)
		}

		fmt.Println(tokenName+" token address", tokenAddr.Hex(), "cap", simulation.TRC2TokenCap)
...
func applyIssuer(trc2Issuer *waihui.TRC2Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		trc2Issuer.TransactOpts.Nonce = big.NewInt(int64(nonce))
		trc2Issuer.TransactOpts.Value = simulation.MinTRC2Apply
		_, err := trc2Issuer.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("trc2Issuer Apply  ", token["name"].(string), err)
		}
...
func applyWaihuiListing(waihuiListing *waihui.WAIHUIListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		waihuiListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		waihuiListing.TransactOpts.Value = simulation.WaihuiListingFee
		_, err := waihuiListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("waihuiListing Apply ", token["name"].(string), err)
		}
		fmt.Println("wait 10s to applyWaihuiListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc2Contract, _ := waihui.NewTRC2(auth, token["address"].(common.Address), client)
			trc2Contract.TransactOpts.Nonce = big.NewInt(int64(nonce))
			_, err := trc2Contract.Transfer(address, big.NewInt(0).Mul(common.BasePrice, big.NewInt(1000000)))
			if err == nil {
